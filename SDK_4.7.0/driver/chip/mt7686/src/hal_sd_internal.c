...
hal_sd_card_type_t sd_check_sd_card_type(void)
{
    ...
    if (SD_COMMAND8_RESPONSE_NO_RESPONSE == sd_information.command8_response) {
		...
		log_hal_info("SD_COMMAND8_RESPONSE_NO_RESPONSE \r\n!");
    } else if (SD_COMMAND8_RESPONSE_VALID == sd_information.command8_response) {
    	log_hal_info("SD_COMMAND8_RESPONSE_VALID \r\n!");
		...
		log_hal_error("SD_COMMAND8_RESPONSE_INVALID \r\n!");
    ...
    while (1) {
        ...
        if (NO_ERROR != status) {
			log_hal_error("sd_send_command55 error \r\n!");
        ...
        if (NO_ERROR != status) {
			log_hal_error("sd_send_command A41 error:%d \r\n!",status);
            sd_information.error_status = status;
            return HAL_SD_TYPE_UNKNOWN_CARD;
        }

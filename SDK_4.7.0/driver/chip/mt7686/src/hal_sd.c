...
hal_sd_status_t hal_sd_init(hal_sd_port_t sd_port, hal_sd_config_t *sd_config)
{
    ...
    if (sd_check_card_type() == HAL_SD_TYPE_UNKNOWN_CARD) {
		...
		log_hal_error("send CMD8 Acmd41 error \r\n!");
        goto error;
    } else {
    ...
    if (sd_information.csd.dsr_imp) {
        status = sd_set_dsr();
        if (NO_ERROR != status) {
			log_hal_error("sd_information.csd.dsr_imp error \r\n!");
            goto error;
        }
    }
    ...
    if (sd_information.card_type == HAL_SD_TYPE_SD_CARD 	  ||
        ...
        status = sd_get_scr();
        if (NO_ERROR != status) {
			log_hal_error("sd_get_scr error \r\n!");
            goto error;
        }
        ...
        if (NO_ERROR != status) {
			log_hal_error("sd_acommand42 error \r\n!");
            goto error;
        }
        ...
        if (NO_ERROR != status) {
			log_hal_error("mmc_set_high_speed error \r\n!");
            goto error;
        }
    ...
    if (NO_ERROR != status) {
		log_hal_error("sd_set_block_length error \r\n!");
        goto error;
    }

...
#define HAL_GPIO_FEATURE_GET_PULL          /* The pin can be configured to get pull state. For more details, please refer to hal_gpio.h. */
#endif
...
/** @brief  The maximum timeout value for UART timeout interrupt, unit is ms.
  */
#define HAL_UART_TIMEOUT_VALUE_MAX  (100000)
...
/** @brief This enum defines the GPIO pull state. */
typedef enum {
    HAL_GPIO_PULL_DISABLE   = 0,              /**<  GPIO disable pull state. */
    HAL_GPIO_PULL_UP        = 1,              /**<  GPIO pull up state. */
    HAL_GPIO_PULL_DOWN      = 2,              /**<  GPIO pull down state. */
    HAL_GPIO_PULL_ERROR     = 3               /**<  GPIO pull up and down, error state. */
} hal_gpio_pull_t;
...
typedef enum {
    ...
    HAL_I2S_FRAME_SYNC_WIDTH_64  = 1,   /**< 64 bits per frame. (Invalid) */
 ...
 */
#define HAL_SPI_MASTER_CLOCK_MAX_FREQUENCY 12000000
...
typedef enum {
    ...
    HAL_GPT_MAX = 5,                       /**< GPT max port \<invalid\> */
    HAL_GPT_MAX_PORT = 5
} hal_gpt_port_t;
...
typedef enum {
    ...
    HAL_PWM_MAX_CHANNEL                    /**< pwm max channel \<invalid\>*/
} hal_pwm_channel_t;
...
/** @brief  Define the platform related WDT restart address.
  */
#define WDT_RESTART_ADDRESS    (0x83080038)

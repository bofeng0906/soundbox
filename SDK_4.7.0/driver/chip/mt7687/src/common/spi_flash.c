...
ATTR_TEXT_IN_TCM int flash_check_device(void)
{
    ...
    if (support_flash_id == SF_TYPE_CUSTOMER) {
        spi_chip_info = (const struct chip_info *)customer_flash_register();
        info = spi_chip_info;
        if (info->id == buf[0] && info->jedec_id == jedec) {
            sf_drvier_status = SF_DRV_READY;
            return 1;
    ...
    sf_drvier_status = SF_DRV_NOT_READY;
    return 0;
}
...
retry:
    ...
    /*if check device status is ready --> switch other task & P/E -->  interrupt occur & suspend flash --> switch current task,
     then device is suspend and can not do a new P/E.*/

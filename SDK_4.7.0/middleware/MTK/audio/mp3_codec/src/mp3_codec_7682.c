...
#include "mp3_codec.h"
static hal_i2s_port_t i2s_port = HAL_I2S_0;//HAL_I2S_1;
#else
...
#define I2S_TX_BUFFER_LENGTH 2048//4608
...
#if 0
    printf("[MP3 Codec ]mp3_codec_share_buffer_write_data_done:: handle->share_buff.write=%d\r\n", handle->share_buff.write);
#endif
}
...
static void mp3_codec_flush(mp3_codec_media_handle_t *handle, int32_t flush_data_flag)
{
    ...
    printf("[MP3 Codec] flush_data_flag = %d\n", (int)handle->flush_data_flag);
}
...
#if 0
    printf("[MP3 Codec]internal_handle->share_buff_size=%u\r\n", internal_handle->share_buff_size);
    printf("[MP3 Codec]internal_handle->pcm_buff_size=%u\r\n", internal_handle->pcm_buff_size);
    printf("[MP3 Codec]internal_handle->working_buff1_size=%u\r\n", internal_handle->working_buff1_size);
    printf("[MP3 Codec]internal_handle->working_buff2_size=%u\r\n", internal_handle->working_buff2_size);
#endif
...
static void mp3_codec_i2s_tx_isr_callback(hal_i2s_event_t event, void *user_data)
{
    ...
    if (1 == internal_handle->media_bitstream_end) {
        printf("[MP3 Codec] isr just return becase media_bitstream_end == 1\r\n");
...
{
    printf("[MP3 Codec]mp3_codec_fill_data_hisr_handler\r\n");
    ...
    do {
        hal_audio_get_stream_out_sample_count(&sample_count);
        printf("[MP3 Codec]sample_count=%u\r\n", sample_count);
...
static void mp3_codec_deocde_hisr_handler(void *data)
{
    ...
    if ((1 == handle->flush_data_flag) && (share_data_amount <= MPEG_AUDIO_FRAME_HEADER_LENGTH)) {
        ...
        if (0 == stream_out_pcm_buffer_data_count) {
            printf("[MP3 Codec] hal_i2s_set_eof_ex(%d)\r\n", i2s_port);
...
static mp3_codec_function_return_state_t mp3_codec_play_internal(mp3_codec_media_handle_t *handle)
{
    ...
    i2c_port = HAL_I2C_MASTER_1;//HAL_I2C_MASTER_0 for nua8810 
    frequency = HAL_I2C_FREQUENCY_200K;//HAL_I2C_FREQUENCY_50K;
...
mp3_codec_media_handle_t *mp3_codec_open(mp3_codec_callback_t mp3_codec_callback)
{
    ...
    if (audio_id > MAX_AUDIO_FUNCTIONS) {
        printf("[MP3 Codec]Audio ID > MAX AUDIO FUNCTIONS\r\n");
        return 0;
    }
#endif

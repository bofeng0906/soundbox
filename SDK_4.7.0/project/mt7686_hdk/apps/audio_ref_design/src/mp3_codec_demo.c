...
#define MP3_CODEC_UT_PLAY_PAUSE_RESUME_10TIMES (1) // for QA test only SDK v4.5.0//maggie
#define MP3_CODEC_DEMO_PLAY_STOP_STEREOMONO_SR_8K_TO_48K (0)//maggie
...
{
    FRESULT res;
    log_hal_info("test_open_file_from_sd() +\r\n");
    if (first_time_f_mount){
        log_hal_info("f_mount() +\r\n");
...
void mp3_codec_demo_play_stop()
{
    ...
    log_hal_info("[MP3 Codec Demo] mp3_codec_set_memory2() +\r\n");
    mp3_codec_set_memory2();
    log_hal_info("[MP3 Codec Demo] mp3_codec_set_memory2() -\r\n");

    if (test_open_file_from_sd(&fdst, _T("SD:/temp/1.mp3")) < 0) {
        log_hal_info("[MP3 Codec Demo] Fail to open SD:/temp/1.mp3.");
        while (1) {
            vTaskDelay(10 / portTICK_RATE_MS);
        }
    }
...
void mp3_codec_demo_play_pause_resume()
{
    ...
    log_hal_info("[MP3 Codec Demo] vTaskDelay(5000 / portTICK_RATE_MS); +\r\n");
    vTaskDelay(5000 / portTICK_RATE_MS);
    log_hal_info("[MP3 Codec Demo] vTaskDelay(5000 / portTICK_RATE_MS); -\r\n");

    log_hal_info("[MP3 Codec Demo] pause +\r\n");
    hdl->pause(hdl);
    log_hal_info("[MP3 Codec Demo] pause -\r\n");    
    ...
    log_hal_info("[MP3 Codec Demo] resume() +\r\n");
    hdl->resume(hdl);
    log_hal_info("[MP3 Codec Demo] resume() -\r\n");
